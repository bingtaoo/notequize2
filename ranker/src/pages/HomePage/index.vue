<template>
  <div>
    ...
    <main class="main" :class="{ blur: whetherBlur }">
      <div class="mask"></div>
      <img @click="switchBackgroundMusic" class="music" :src="musicIconSrc" alt="music" />
      <router-link to="/invitation">
        <button class="invite-code head-btn">填写邀请码</button>
      </router-link>
      <router-link to="/rank">
        <button class="rank-list head-btn">排行榜&nbsp;
      ...
      <section class="info-panel">
        ...
        <!--三个按钮-->
        <BeginButton class="practice-btn" @click.enter="play('practice')">
          <span class="text">练习模式 (每天三次机会)</span>
        </BeginButton>
        <BeginButton class="normal-btn" @click.enter="play('normal')">
          <img class="begin" src="../../assets/icon/begin.svg" alt="begin">
          <span class="text">开始答题</span>
        </BeginButton>
        <BeginButton class="activity-btn" @click.enter="play('activity')" :propsStyle="{ backgroundColor: '#198cf9' }">
...
import { END_GAME, GET_CACHE, PLAY_GAME, SET_USER_INFO, SWITCH_MUSIC } from '@/store/mutation-types';
