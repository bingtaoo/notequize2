<template>
  <main class="main">
    ...
    <svg class="animation clock" v-if="isAnswered && isRight" version="1.1" xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 130.2 130.2">
      <circle class="path circle" fill="none" stroke="#73AF55" stroke-width="6" stroke-miterlimit="10" cx="65.1"
        cy="65.1" r="62.1">
      </circle>
      <polyline class="path check" fill="none" stroke="#73AF55" stroke-width="10" stroke-linecap="round"
        stroke-miterlimit="10" points="100.2,40.2 51.5,88.8 29.8,67.5 ">
      </polyline>
    </svg>
    <svg class="animation clock" v-else-if="isAnswered" version="1.1" xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 130.2 130.2">
    ...
    <CountdownTimer v-else class="clock" :rotateDeg="(totalTime - restTime) / totalTime * 360" :text="restTime" />
    <div class="background"></div>
    <transition name="slide">
      <div class="question" v-if="show">{{ question.questionTitle }}</div>
    </transition>
    <ul class="options" v-if="show">
      <OptionSetting v-for="(item, index) in question.questionOptions" :key="question.questionId + index"
        :optionsInfo="optionsInfo[index]" :rate="item.rate" @click.enter="judgeResult(index)">
        {{ item.content }}
      </OptionSetting>
    </ul>
    <StatusBar class="bottom-bar" :count="questionCount" :current="currentQuestion" />
    <ResultPage v-if="isEnd" :result="isRight" :title="showMessage.title" :text="showMessage.text"
      :score="totalScore" />
